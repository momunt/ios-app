...
#import "MMNT_AddPhoneNumberVC.h"
#import "MMNTApiCommuniator.h"
#import "MMNT_SharedVars.h"
#import "MMNT_VerifyPhoneNumberVC.h"


@interface MMNT_AddPhoneNumberVC ()<UITextFieldDelegate, CountryPickerDelegate, UIImagePickerControllerDelegate, UIActionSheetDelegate>{
...
@implementation MMNT_AddPhoneNumberVC
...
- (void)viewDidLoad {
    ...
    _animatedViews = @[_prompt1, _prompt2, _phoneField.superview, _termsText];
    ...
    NSString *text = @"We will not display your phone number to the other users. Read our Privacy Policy to learn more";
...
-(void)keyboardWillShow:(NSNotification *)note{
    ...
    _prompt2.transform = CGAffineTransformMakeTranslation(0, -50);
    
    _errorMsg.center = CGPointMake(_errorMsg.center.x, _prompt1.center.y+100);
...
-(void)keyboardWillHide:(NSNotification *)note{
    ...
    _prompt1.transform = CGAffineTransformIdentity;
    _prompt2.transform = CGAffineTransformIdentity;
...
//        [self dismissKeyboard];
        ...
        if(![_phoneField.text isEqualToString:@""]){
            // submit number and go to ferification of no error
            NSString *phoneStrNospaces = [NSString stringWithFormat:@"%@%@",_callCode, _phoneField.text];
            
            [[MMNTApiCommuniator sharedInstance] registerNumber:phoneStrNospaces completion:^(BOOL done) {
                if(!done){
                    [_exitOrSubmit setTitle:@"submit" forState:UIControlStateNormal];
                    [self setError:@"Oops, this number is invalid or registered" onField:_phoneField]; //Please enter a valid number - doesnt fit on the screen
                }else{
    ...
    [UIView animateWithDuration:0.3
         ...
         usingSpringWithDamping:0.6
          initialSpringVelocity:1
                     ...
                     animations:^{
                         _errorMsg.center = CGPointMake(_errorMsg.center.x, _prompt1.center.y+63);
                         _errorMsg.alpha = 1.0;
                     }
                     completion:^(BOOL finished) {
                         NSLog(@"here");
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if([segue.destinationViewController isKindOfClass: [MMNT_VerifyPhoneNumberVC class] ]){
        
        MMNT_VerifyPhoneNumberVC *toVC = segue.destinationViewController;

...
-(void)setMask:(UIImage *)maskImage{
    _hasMask = YES;
...
//        [UIView animateWithDuration:0.3 delay:_appearDelay options:UIViewAnimationOptionCurveEaseInOut animations:^{
//            [self setTransform:CGAffineTransformMakeScale(0, 0)];
//        } completion:^(BOOL finished) {}];
    }
}
-(void)downloadFromURL:(NSURL *)imageURL completionBlock:(void (^)(BOOL succeeded, UIImage *image))completionBlock
{
    UIImage *image = [[AsyncImageLoader sharedLoader].cache objectForKey:imageURL];
    if (image)
    {
...
//    if(_shouldCheckCell){
//        UICollectionViewCell *updateCell = (id)[_collectionView cellForItemAtIndexPath:_indexPath];
//        if (!updateCell){
//            return;
//        }
//    }
    ...
    if(image != self.image && _animationType==AsyncImageZoomIn){
        [UIView animateWithDuration:0.3 delay:_appearDelay options:UIViewAnimationOptionCurveEaseInOut animations:^{
            [self setTransform:CGAffineTransformMakeScale(0, 0)];
        } completion:^(BOOL finished) {
            [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
                ...
                _animationType=AsyncImageNormal; // only animate each image once
            }];
    ...
    if(_hasMask){
        ...
        UIImage *maskedImage = [self maskImage:image withMask:_maskImage];
        super.image = maskedImage;
    }
    else if(_shouldCenter){
        CGFloat h = image.size.width * self.frame.size.height / self.frame.size.width;
        CGFloat offset = (image.size.width - h)/2;
...
-(void)applyMask{
    ...
    mask.contents = (id)[_maskImage CGImage];

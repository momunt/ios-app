//
//  MMNT_NoInstagramSignInViewController.m
//  Momunt
//
...
#import "MMNT_NoInstagramSignInViewController.h"
...
@interface MMNT_NoInstagramSignInViewController () <UITextFieldDelegate, CountryPickerDelegate, UIImagePickerControllerDelegate, UIActionSheetDelegate>{
...
@implementation MMNT_NoInstagramSignInViewController
...
- (void)viewDidLoad {
    ...
    UITapGestureRecognizer* tap1 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(pressedCameraButton:)];
    tap1.numberOfTapsRequired = 1;
    [_prompt addGestureRecognizer:tap1];
    tap1.delegate = self;
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if(textField==_usernameField){
        // only alphanumerical in username
...
-(void)keyboardWillShow:(NSNotification *)note{
    ...
    if(_currentView==_usernameField.superview){
        _passwordField.superview.alpha = 0.0;
    }
    if(_currentView==_passwordField.superview){
    ...
    _cameraButton.transform = CGAffineTransformTranslate(_currentView.transform, 0, correction);
    ...
    _prompt.transform = CGAffineTransformMakeTranslation(0, -30);
...
-(void)keyboardWillHide:(NSNotification *)note{
    ...
    // may need to move the profile pic and prompt
    _prompt.transform = CGAffineTransformIdentity;
    ...
    _passwordField.superview.alpha = 1.0;
    
    if(![_usernameField.text isEqualToString:@""] && ![_passwordField.text isEqualToString:@""] && _selectedProfile){
        [_exitOrSubmit setTitle:@"submit" forState:UIControlStateNormal];
        //        [self loginInstagram];
    }else{
        [_exitOrSubmit setTitle:@"exit" forState:UIControlStateNormal];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if([segue.destinationViewController isKindOfClass: [MMNT_VerificationVC class] ] || [segue.sourceViewController isKindOfClass: [MMNT_VerificationVC class] ]){
        _animatedViews = [[NSArray alloc] initWithObjects:_prompt, _profile, _usernameField.superview, _passwordField.superview, _phoneField.superview, _termsText ,nil];
...
- (IBAction)pressedExitOrSubmit:(id)sender {
    ...
    if([action isEqualToString:@"exit"]){
        ...
        if(![_usernameField.text isEqualToString:@""] && ![_passwordField.text isEqualToString:@""] && _selectedProfile && _phoneField.text.length==0 && _keyboardActive){
        // filled out everything exept for optional phone number. Slide down keyboard
            [self dismissKeyboard];
        }
        else if(![_usernameField.text isEqualToString:@""] && ![_passwordField.text isEqualToString:@""] && _selectedProfile && _phoneField.text.length==0 && !_keyboardActive){
        ...
        else if(![_usernameField.text isEqualToString:@""] && ![_passwordField.text isEqualToString:@""] && !_selectedProfile){
            // forgot profile!
            [self setErrorOnProfile];
        }
        else if(![_usernameField.text isEqualToString:@""] && ![_passwordField.text isEqualToString:@""] && _selectedProfile && ![_phoneField.text isEqualToString:@""]){
            ...
            [_apicommunicator registerUser:_usernameField.text password:_passwordField.text phone:phoneStrNospaces profile:_profile.image instagramToken:nil fullName:@"" completion:^(NSDictionary *result) {
...
- (void)dismissKeyboard {
    if(_inCountryPicker){
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            _passwordField.superview.alpha = 1.0f;
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    _profile.layer.borderColor = [UIColor colorWithRed:1 green:0 blue:0 alpha:1].CGColor;
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if(alertView.tag==100){
        if(buttonIndex==0)
        {
            ...
            [_apicommunicator registerUser:_usernameField.text password:_passwordField.text phone:nil profile:_profile.image instagramToken:nil fullName:@"" completion:^(NSDictionary *result) {
...
/*
 Set error
 */
-(void)setErrorOnProfile{
    _errorMsg.alpha = 0.0;
    _errorMsg.hidden = NO;
    _errorMsg.text = @"please upload a profile photo!";
